...
#' @title Infection process for vaccine model with variants of concern (multi-dose, no types)
#'
...
#' @export
infection_process_voc <- function(parameters, variables, events, dt) {
  ...
  return(
    ...
    function(timestep) {
      ...
      if (infectious$size() > 0) {
        ...
        t <- ceiling(timestep * dt)

        voc_prop <- matrix(parameters$voc_trajectory[t, ], ncol = 1)

        # Group infection by age
        ages <- variables$discrete_age$get_values(infectious)
        inf_ages <- tab_bins(a = ages, nbins = parameters$N_age)

        # calculate FoI for each age group and voc (lambda has dims vox X age)
        m <- get_contact_matrix(parameters)
        lambda <- voc_prop %*% (parameters$beta_set[t] * as.vector(m %*% inf_ages))

        # S: persons at risk of infection
