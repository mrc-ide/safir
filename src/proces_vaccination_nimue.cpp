...
Rcpp::XPtr<process_t> vaccination_process_nimue_cpp_internal(
  ...
  std::vector<double> pr(N_age);
  Rcpp::IntegerMatrix vaccination_target_mat(N_prioritisation_steps , N_age);
  std::vector<int> vaccine_target_vec(N_age);
  Rcpp::NumericMatrix vaccine_coverage_mat = Rcpp::as<Rcpp::NumericMatrix>(parameters["vaccine_coverage_mat"]);
  std::vector<int> vaccination_target(N_age);

  return Rcpp::XPtr<process_t>(
    new process_t([parameters, states, eligible, vaccinated, empty, discrete_age, v0_to_v1v2, dt, vaxx_states, N_age, N_prioritisation_steps, vaccine_coverage_mat, &pr, &vaccination_target_mat, &vaccine_target_vec, &vaccination_target](size_t t){
