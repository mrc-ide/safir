...
graph <-
  ...
  add_node(label = "S_{0}") %>%
  add_node(label = "S_{1}") %>%
  add_node(label = "E1_{0}") %>%
  add_node(label = "E1_{1}")
...
graph <- 
  graph %>%
  add_edge(from = 1, to = 2, edge_aes = edge_aes(label="vaccination")) %>%
  add_edge(from = 3, to = 4, edge_aes = edge_aes(label="vaccination")) %>%
  add_edge(from = 1, to = 3, edge_aes = edge_aes(label="infection (v0)")) %>%
  add_edge(from = 2, to = 4, edge_aes = edge_aes(label="infection (v1)")) %>%
  add_edge(from = 1, to = 4, edge_aes = edge_aes(label="infection + vaccinated",style="dashed"))
...
graph <-
  ...
  add_edge(from=2,to=3, edge_aes = edge_aes(label="event scheduled")) %>%
  add_edge(from=3,to=4, edge_aes = edge_aes(label="event scheduled"))
