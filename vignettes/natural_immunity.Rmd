...
# ab boost for each infection (maxes out at 51 infections)
...
This part is also exactly the same as the "Vaccine model with multiple doses" except the following places:
  ...
  2. `attach_event_listeners_natural_immunity` is called to add additional event listeners to `events` list needd to antibody boosting
  ...
  4. `natural_immunity_ab_titre_process` is added to the `processes` list to update antibody titre from vaccines or natural immunity.
